...
window.saveProfile = async function () {
  ...
  try {
    const { error } = await supabase
      .from("users").update({ username, team, bio }).eq("uid", currentUser.uid);
    if (error) throw error;
...
window.handleAvatarUpload = async function (event) {
  ...
  try {
    ...
    const { error: updateErr } = await supabase
      .from("users").update({ avatar: url }).eq("uid", currentUser.uid);
...
window.submitPost = async function () {
  ...
  try {
    ...
    await supabase.from("users").update({ postcount: newCount }).eq("uid", currentUser.uid);
...
function buildPostCard(postId, p, isProfile = false) {
  ...
  const isMyPost   = currentUser && p.authorid === currentUser.uid;
    ...
    <div class="post-card" id="post-${postId}">
      <div class="flex items-start gap-3 mb-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-mist text-xs">${currentUserDoc?.team || ""}</p>
        </div>
        ${isMyPost ? `<button onclick="deletePost('${postId}')" class="text-mist hover:text-coral ml-2"><i data-lucide="trash-2" class="w-4 h-4"></i></button>` : ""}
      </div>
      ...
      <div class="flex items-center gap-6 text-xs text-mist border-t border-line pt-3">
        <button onclick="toggleLike('${postId}', ${JSON.stringify(p.likes || [])})"
          class="flex items-center gap-1.5 hover:text-lime transition ${liked ? "text-lime" : ""}">
...
window.toggleLike = async function (postId, currentLikes) {
  ...
  if (currentLikes.includes(uid)) {
    ...
    const { data: post } = await supabase
      .from("posts").select("authorid").eq("id", postId).single();
...
function appendChatMessage(container, m) {
  const isMine    = currentUser && m.authorId === currentUser.uid;
  ...
  wrapper.innerHTML = `
    ...
    <div class="max-w-xs">
      ...
      <p class="text-xs text-mist mt-1 ${isMine ? "text-right" : ""}">${ts}</p>
    </div>`;
...
async function loadDMConversations() {
  ...
  try {
    ...
    data.forEach(dm => {
      ...
      const otherName   = dm.participantNames?.[otherId] || "Unknown";
      const otherAvatar = dm.participantAvatars?.[otherId] || `https://ui-avatars.com/api/?name=${encodeURIComponent(otherName)}&background=0f1f17&color=b5ff47&size=40`;
...
window.openDMById = async function (dmId, otherId, otherName, otherAvatar) {
  ...
  if (!dmId) {
    dmId = [currentUser.uid, otherId].sort().join("_");
  }
  ...
  if (!existingDm) {
    await supabase.from("dms").insert({
      ...
      participantNames:  {
        [currentUser.uid]: currentUserDoc?.username || "Me",
        [otherId]:         otherName,
      },
      participantAvatars: {
        [currentUser.uid]: currentUserDoc?.avatar || "",
        [otherId]:         otherAvatar,
      },
      lastMessage:   "",
       timestamp:     new Date().toISOString(),
...
window.sendDM = async function () {
  ...
  input.value = "";
  const dmId = [currentUser.uid, activeDMUser.id].sort().join("_");
