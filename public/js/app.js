...
window.submitPost = async function () {
  ...
  try {
    ...
    showToast("Failed to post: " + (err.message || err.details || JSON.stringify(err)), "error");
  }
};
...
async function loadDMConversations() {
  ...
  try {
    const { data, error } = await supabase
      .from("dms").select("*")
      .or(`user1id.eq.${currentUser.uid},user2id.eq.${currentUser.uid}`)
      .order("created_at", { ascending: false })
      .limit(20);
    ...
    for (const dm of data) {
      const otherId = dm.user1id === currentUser.uid ? dm.user2id : dm.user1id;
...
window.openDMById = async function (dmId, otherId, otherName, otherAvatar) {
  ...
  if (!existingDm) {
    await supabase.from("dms").insert({
      id: dmId,
      user1id:    [currentUser.uid, otherId].sort()[0],
      user2id:    [currentUser.uid, otherId].sort()[1],
      lastmsg:    "",
      created_at: new Date().toISOString(),
    });
  }
