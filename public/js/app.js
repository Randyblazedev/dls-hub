...
const NotifSounds = (() => {
  let ctx = null;
  const PREF_KEY = 'dls-sounds-enabled';
  const isEnabled = () => localStorage.getItem(PREF_KEY) !== 'off';
  ...
  /** Play a sequence of { freq, duration, delay, type, gain } notes */
  function play(notes) {
    if (!isEnabled()) return;
    try {
      const ac = getCtx();
      if (ac.state === 'suspended') ac.resume();
      notes.forEach(({ freq, duration = 0.12, delay = 0, type = 'sine', gain = 0.18 }) => {
  ...
  return {
    /** Incoming DM or chat message — bright two-tone ping */
    message() {
      play([
        { freq: 880, duration: 0.10, delay: 0,    type: 'sine', gain: 0.16 },
        { freq: 1175, duration: 0.14, delay: 0.10, type: 'sine', gain: 0.16 },
      ]);
    },
    /** Notification (like, comment, etc.) — soft single chime */
    notification() {
      play([
        { freq: 660, duration: 0.15, delay: 0,    type: 'sine', gain: 0.14 },
        { freq: 880, duration: 0.12, delay: 0.08, type: 'triangle', gain: 0.10 },
      ]);
    },
    /** Success toast — ascending two-note */
    success() {
      play([
        { freq: 523, duration: 0.10, delay: 0,    type: 'sine', gain: 0.12 },
        { freq: 659, duration: 0.12, delay: 0.08, type: 'sine', gain: 0.12 },
      ]);
    },
    /** Error toast — descending two-note */
    error() {
      play([
        { freq: 440, duration: 0.12, delay: 0,    type: 'square', gain: 0.08 },
        { freq: 330, duration: 0.15, delay: 0.10, type: 'square', gain: 0.08 },
      ]);
    },
    /** Toggle sound on/off */
    toggle() {
...
window.toggleSoundPref = function () {
  const now = NotifSounds.toggle();
  showToast(now ? 'Sounds enabled' : 'Sounds muted');
};
    ...
    try {
      ...
      data.forEach(m => {
        if (!renderedIds.has(m.id)) {
          ...
          appendChatMessage(container, m, true);
        }
      });
...
function appendChatMessage(container, m, isLive) {
  ...
  if (isLive && !isMine) NotifSounds.message();
...
window.openDMById = async function (dmId, otherId, otherName, otherAvatar) {
  ...
  async function fetchDMMessages() {
    ...
    data.forEach(m => {
      ...
      if (!isMine && !renderedDmIds.has(m.id)) NotifSounds.message();
...
function showToast(msg, type = "success") {
  ...
  if (type === "error") NotifSounds.error(); else NotifSounds.success();
