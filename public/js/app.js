...
window.navigate = function (page) {
  ...
  if (page === "predictor")     loadPredictor();
...
function showCreateSquad() {
  document.getElementById('create-squad-form').classList.toggle('hidden');
}

window.createSquad = async function () {
  if (!currentUser) { showToast('Please log in', 'error'); return; }
  const name = document.getElementById('squad-name').value.trim();
  const formation = document.getElementById('squad-formation').value;
...
window.loadSquads = async function () {
  try {
    const { data, error } = await supabase.from('squads').select('*, users!squads_userid_fkey(username)').order('created_at', { ascending: false }).limit(20);
    if (error) throw error;
    const container = document.getElementById('squad-list');
    if (!data || !data.length) {
      container.innerHTML = '<p class="text-mist text-center py-8">No squads yet. Create one!</p>';
      return;
    }
    container.innerHTML = data.map(s => `
      <div class="bg-turf border border-line rounded-2xl p-5 hover:border-lime/30 transition">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-ice font-bold text-lg">${escHtml(s.name)}</h3>
          <span class="text-lime text-xs font-mono">${s.formation}</span>
        </div>
        <p class="text-mist text-xs">by ${escHtml(s.users?.username || 'Unknown')} • ${(s.likes?.length || 0)} likes</p>
      </div>
    `).join('');
    lucide.createIcons();
  } catch (err) {
...
window.loadAchievements = async function () {
  if (!currentUser) return;
  try {
    const { data } = await supabase.from('achievements').select('*').eq('userid', currentUser.id);
    const earned = (data || []).map(a => a.type);
    const container = document.getElementById('achievements-list');
    container.innerHTML = ACHIEVEMENT_DEFS.map(a => {
      const isEarned = earned.includes(a.type);
      return `
        <div class="bg-turf border border-line rounded-xl p-4 text-center ${isEarned ? 'border-lime/40' : 'opacity-50'}">
          <div class="text-3xl mb-2">${a.icon}</div>
          <h3 class="text-ice text-sm font-bold">${a.title}</h3>
          <p class="text-mist text-xs mt-1">${a.description}</p>
          ${isEarned ? '<span class="text-lime text-xs font-bold">✅ Earned</span>' : '<span class="text-mist text-xs">🔒 Locked</span>'}
        </div>
      `;
    }).join('');
  } catch (err) {
...
async function grantAchievement(type) {
  if (!currentUser) return;
  const def = ACHIEVEMENT_DEFS.find(a => a.type === type);
  if (!def) return;
  // Check if already earned
  const { data } = await supabase.from('achievements').select('id').eq('userid', currentUser.id).eq('type', type).limit(1);
  if (data && data.length) return;
  // Grant it
  await supabase.from('achievements').insert({ userid: currentUser.id, type, title: def.title, description: def.description, icon: def.icon });
...
window.loadChallenges = async function () {
  try {
    const { data, error } = await supabase.from('challenges').select('*').eq('status', 'active').order('created_at', { ascending: false });
    if (error) throw error;
    const container = document.getElementById('challenges-list');
    if (!data || !data.length) {
      container.innerHTML = '<p class="text-mist text-center py-8">No active challenges. Check back later!</p>';
      return;
    }
    container.innerHTML = data.map(c => `
      <div class="bg-turf border border-line rounded-2xl p-5">
        <div class="flex items-center justify-between mb-2">
          <h3 class="text-ice font-bold">${escHtml(c.title)}</h3>
          <span class="text-lime text-xs font-bold">+${c.reward_points} pts</span>
        </div>
        <p class="text-mist text-sm">${escHtml(c.description || '')}</p>
        <p class="text-mist text-xs mt-2">Type: ${c.challenge_type} • Ends: ${new Date(c.end_date || Date.now() + 7*86400000).toLocaleDateString()}</p>
      </div>
    `).join('');
  } catch (err) {
...
window.loadRatings = async function () {
  try {
    const { data, error } = await supabase.from('player_ratings').select('*, users!player_ratings_userid_fkey(username)').order('created_at', { ascending: false }).limit(30);
    if (error) throw error;
    const container = document.getElementById('ratings-list');
    if (!data || !data.length) {
      container.innerHTML = '<p class="text-mist text-center py-8">No ratings yet. Be the first!</p>';
      return;
    }
    container.innerHTML = data.map(r => `
      <div class="bg-turf border border-line rounded-xl p-4">
        <div class="flex items-center justify-between mb-2">
          <h3 class="text-ice font-bold">${escHtml(r.player_name)}</h3>
          <span class="text-lime text-lg font-bold">${r.rating}/10</span>
        </div>
        ${r.review ? `<p class="text-mist text-sm">${escHtml(r.review)}</p>` : ''}
        <p class="text-mist text-xs mt-2">by ${escHtml(r.users?.username || 'Unknown')}</p>
      </div>
    `).join('');
  } catch (err) {
...
window.submitRating = async function () {
  if (!currentUser) { showToast('Please log in', 'error'); return; }
  const player = document.getElementById('rating-player').value.trim();
  ...
  try {
    ...
    document.getElementById('rating-player').value = '';
    document.getElementById('rating-review').value = '';
    loadRatings();
    grantAchievement('rater');
  } catch (err) {
...
window.loadPredictor = async function () {
  const container = document.getElementById('predictor-content');
  container.innerHTML = `
    <div class="bg-turf border border-line rounded-2xl p-6">
      <h3 class="text-ice font-bold text-lg mb-4">⚽ Predict a Match</h3>
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label class="form-label">Your Team</label>
          <input id="pred-team1" class="form-input" placeholder="e.g. Barcelona" />
        </div>
        <div>
          <label class="form-label">Opponent</label>
          <input id="pred-team2" class="form-input" placeholder="e.g. Madrid" />
        </div>
      </div>
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label class="form-label">Your Score</label>
          <input id="pred-score1" type="number" class="form-input" placeholder="0" min="0" max="20" />
        </div>
        <div>
          <label class="form-label">Their Score</label>
          <input id="pred-score2" type="number" class="form-input" placeholder="0" min="0" max="20" />
        </div>
      </div>
      <button onclick="submitPrediction()" class="btn-lime text-sm px-6 py-2">Submit Prediction (+10 pts)</button>
    </div>
    <div class="bg-turf border border-line rounded-2xl p-4 mt-4">
      <h4 class="text-ice font-bold text-sm mb-3">🏆 Recent Predictions</h4>
      <div id="predictions-list" class="text-mist text-sm">Loading...</div>
    </div>
  `;
};
...
window.submitPrediction = async function () {
  if (!currentUser) { showToast('Please log in', 'error'); return; }
  const team1 = document.getElementById('pred-team1').value.trim();
  const team2 = document.getElementById('pred-team2').value.trim();
  const score1 = document.getElementById('pred-score1').value;
  const score2 = document.getElementById('pred-score2').value;
  if (!team1 || !team2 || !score1 || !score2) { showToast('Fill in all fields', 'error'); return; }
  showToast('Prediction submitted! +10 pts 🏆');
  grantAchievement('challenger');
};
...
window.loadReplays = async function () {
  const container = document.getElementById('replays-list');
  container.innerHTML = `
    <div class="bg-turf border border-line rounded-2xl p-6 text-center">
      <div class="text-4xl mb-3">🎮</div>
      <h3 class="text-ice font-bold mb-2">Match Replays</h3>
      <p class="text-mist text-sm mb-4">Share your best DLS match highlights and screenshots!</p>
      <p class="text-mist text-xs">Coming soon — upload match screenshots to share with the community.</p>
    </div>
  `;
};
